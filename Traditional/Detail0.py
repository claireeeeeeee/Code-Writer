...
def get_coder1(entry_point, prompt, analyst_say, task_id):
    messages = [
        ...
        {'role': 'user', 'content': f"The Requirement Analyst Plan from analyst is {analyst_say}. The function name should be {entry_point}. Output in Python code format."},
        {'role': 'assistant', 'content': f"""
This plan will outline:
1. The package/module you expect to use may contain.
2. The name of function you must follow. 
...
def get_coder2(test_say, code_say, task_id):
    coder_description = f"""
...
"""
    ...
    save_history(task_id, 'Coder2', result_say)
    return result_say
...
def code_generation(api_key, task_id, prompt, entry_point):
    ...
    code_say = get_coder1(entry_point, prompt, analyst_say, task_id)
    test_say = get_tester(code_say, prompt, task_id)
    code_say = get_coder2(test_say, code_say, task_id)
